...
extern int RV_loggingOn()
{
    int result = OK;
    LG_logEntry(__func__, NULL);
    SAFE_INVOKE(LG_setLogging(pthread_self(), true), result,
            RV_LOGGING_ON_FAILED)
    LG_logExit(__func__, result, NULL);
    return result;
}
...
extern int RV_loggingOff()
{
    int result = OK;
    LG_logEntry(__func__, NULL);
    SAFE_INVOKE(LG_setLogging(pthread_self(), false), result,
            RV_LOGGING_OFF_FAILED)
    LG_logExit(__func__, result, NULL);
    return result;
}
...
extern int RV_getLineSensors(RV_LineSensors* lineSensors)
{
    ...
    REG_IR_LINE_W };
    int* destinations[] =  {
            &lineSensors->N.ambient, &lineSensors->E.ambient,
            &lineSensors->S.ambient,&lineSensors->W.ambient,
            &lineSensors->N.active,&lineSensors->E.active,
            &lineSensors->S.active,&lineSensors->W.active
    };

    LG_logEntry(__func__, "lineSensors: %p", lineSensors);
    for (unsigned int i = 0;
         (result == OK) && (i < sizeof(registers) / sizeof(int));
         i++)
    {
        SAFE_INVOKE(REG_read16(registers[i], &v), result,
                RV_GET_ANALOGLINE_FAILED)
        if (result == OK)
        {
            *(destinations[i]) = v;
        }
    }
    LG_logExit(__func__, result,
            "lineSensors N: (%d, %d), E: (%d, %d), S: (%d, %d), W: (%d, %d)",
            lineSensors->N.active, lineSensors->N.ambient,
            lineSensors->E.active, lineSensors->E.ambient,
            lineSensors->S.active, lineSensors->S.ambient,
            lineSensors->W.active, lineSensors->W.ambient);
    return result;
}
...
extern int RV_getCollisionSensors(RV_CollisionSensors* collisionSensors)
{
    ...
    int* destinations[] =
    {
            &collisionSensors->NE.ambient, &collisionSensors->SE.ambient,
            &collisionSensors->SW.ambient, &collisionSensors->NW.ambient,
            &collisionSensors->NE.active, &collisionSensors->SE.active,
            &collisionSensors->SW.active, &collisionSensors->NW.active
    };

    LG_logEntry(__func__, "collisionSensors: %p", collisionSensors);
    for (unsigned int i = 0;
         (result == OK) && (i < sizeof(registers) / sizeof(int));
         i++)
    {
    ...
    LG_logExit(__func__, result,
            "lineSensors N: (%d, %d), E: (%d, %d), S: (%d, %d), W: (%d, %d)",
            collisionSensors->NE.active, collisionSensors->NE.ambient,
            collisionSensors->SE.active, collisionSensors->SE.ambient,
            collisionSensors->SW.active, collisionSensors->SW.ambient,
            collisionSensors->NW.active, collisionSensors->NW.ambient);
    return result;
}
